...
The `@next/mdx` package, and related packages, are used to configure Next.js so it can process markdown and MDX. **It sources data from local files**, allowing you to create pages with a `.md` or `.md` extension, directly in your `/pages` or `/app` directory.
...
This allows `.md` and `.md` files to act as pages, routes, or imports in your application.
...
```mdx filename="markdown/welcome.md" switcher
import { MyComponent } from 'my-component'
...
```mdx filename="content/blog-post.md" switcher
...
import BlogPost, { metadata } from '@/content/blog-post.md'
