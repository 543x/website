...
```yaml filename="required-fields.md"
---
title: Page Title
...
```yaml filename="optional-fields.md"
---
...
To avoid content duplication and risk the content becoming out of sync, we use the `source` field to pull content from one page into another. For example, the `<Link>` component behaves _mostly_ the same in **App** and **Pages**. Instead of duplicating the content, we can pull the content from `app/.../link.md` into `pages/.../link.md`:

```mdx filename="app/.../link.md"
---
title: <Link>
