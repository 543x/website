...
Admonitions are implemented with a [Remark plugin](./markdown-features-plugins.md). The plugin is designed to be configurable. To customize the Remark plugin for a specific content plugin (docs, blog, pages), pass the options through the `admonitions` key.
