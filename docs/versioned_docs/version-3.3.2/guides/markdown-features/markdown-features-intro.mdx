...
Docusaurus compiles both `.md` and `.md` files to React components using the MDX compiler, but **the syntax can be interpreted differently** depending on your settings.
...
It is possible to **opt-in for CommonMark** using the [`siteConfig.markdown.format`](../../api/docusaurus.config.js.md#markdown) setting or the `format: md` front matter.

:::tip how to use CommonMark

If you plan to use CommonMark, we recommend the [`siteConfig.markdown.format: 'detect'`](../../api/docusaurus.config.js.md#markdown) setting. The appropriate format will be selected automatically, based on file extensions:

- `.md` files will use the CommonMark format
- `.md` files will use the MDX format
...
The Markdown syntax `![message](url)` only declaratively tells Docusaurus that an image needs to be inserted here, but we may do other things like transforming a [file path to URL path](./markdown-features-assets.md#images), so the generated markup may differ from the output of other Markdown renderers, or a naïve hand-transcription to the equivalent JSX/HTML code.

In general, you should only assume the _semantics_ of the markup (` ``` ` fences become [code blocks](./markdown-features-code-blocks.md); `>` becomes [quotes](#quotes), etc.), but not the actual compiled output.
...
Use the [Markdown config `parseFrontMatter` function](../../api/docusaurus.config.js.md#markdown) to provide your own front matter parser, or to enhance the default parser.
