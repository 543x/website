...
Refer to the Getting Started [**Configuration**](../configuration.md) for examples.
...
With a [TypeScript](../typescript-support.md) Docusaurus codebase your config file may be called `docusaurus.config.ts`. The syntax is broadly identical to the `js` config file with the addition of types. You can see an example on the [Docusaurus Website](https://github.com/facebook/docusaurus/blob/main/website/docusaurus.config.ts) itself.
...
- [**TypeScript**](../typescript-support.md#typing-config)
...
Refer to [**Syntax to declare `docusaurus.config.js`**](../configuration.md#syntax-to-declare-docusaurus-config) for a more exhaustive list of examples and explanations.
...
Refer to the [deployment guide](../deployment.md) and [slorber/trailing-slash-guide](https://github.com/slorber/trailing-slash-guide) to choose the appropriate setting.
...
| --- | --- | --- | --- |
| `format` | `'mdx' \| 'md' \| 'detect'` | `'mdx'` | The default parser format to use for Markdown content. Using 'detect' will select the appropriate format automatically based on file extensions: `.md` vs `.md`. |
...
| `parseFrontMatter` | `ParseFrontMatter` | `undefined` | Gives you the ability to provide your own front matter parser, or to enhance the default parser. Read our [front matter guide](../guides/markdown-features/markdown-features-intro.md#front-matter) for details. |
