...
The [feed feature](../../blog.md#feed) works by extracting the build output, and is **only active in production**.
...
You can configure this plugin through the [preset options](../../using-plugins.md#docusauruspreset-classic).
...
Markdown documents can use the following Markdown [front matter](../../guides/markdown-features/markdown-features-intro.md#front-matter) metadata fields, enclosed by a line `---` on either side.
...
| Name | Type | Default | Description |
| --- | --- | --- | --- |
| `authors` | `Authors` | `undefined` | List of blog post authors (or unique author). Read the [`authors` guide](../../blog.md#blog-post-authors) for more explanations. Prefer `authors` over the `author_*` front matter fields, even for single author blog posts. |
...
| `date` | `string` | File name or file creation time | The blog post creation date. If not specified, this can be extracted from the file or folder name, e.g, `2021-04-15-blog-post.md`, `2021-04-15-blog-post/index.md`, `2021/04/15/blog-post.md`. Otherwise, it is the Markdown file creation time. |
...
import TagsFileApiRefSection from './_partial-tags-file-api-ref-section.md';
...
This file can contain a list of predefined [global blog authors](../../blog.md#global-authors). You can reference these authors by their keys in Markdown files thanks to the [`authors` front matter](#markdown-front-matter).
