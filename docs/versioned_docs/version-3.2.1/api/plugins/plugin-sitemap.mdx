...
The `lastmod` option will only output a sitemap `<lastmod>` tag if plugins provide [route metadata](../plugin-methods/lifecycle-apis.md#addRoute) attributes `sourceFilePath` and/or `lastUpdatedAt`.
