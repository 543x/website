...
- Create a versioned sidebars file based from your current [sidebar](./sidebar/index.md) configuration (if it exists) - saved as `versioned_sidebars/version-[versionName]-sidebars.json`.
...
If you deploy your site on a Jamstack provider (e.g. [Netlify](../../deployment.md)), the provider will save each production build as a snapshot under an immutable URL. You can include archived versions that will never be rebuilt as external links to these immutable URLs. The Jest website and the Docusaurus website both use such pattern to keep the number of actively built versions low.
