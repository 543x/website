...
While Docusaurus parses both `.md` and `.md` files using MDX, some of the syntaxes are treated slightly differently by third-party tools. For the most accurate parsing and better editor support, we recommend using the `.md` extension for files containing MDX syntax.
...
This behavior is different from Docusaurus 1. See also [Migrating from v1 to v2](../../migration/migration-manual.md).
