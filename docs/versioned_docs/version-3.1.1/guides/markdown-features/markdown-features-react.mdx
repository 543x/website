...
Prettier, the most popular formatter, [supports only the legacy MDX v1](https://github.com/prettier/prettier/issues/12209). If you get an unintentional formatting result, you may want to add `{/* prettier-ignore */}` before the problematic area, or add `*.md` to your `.prettierignore`, until Prettier has proper support for MDX v3. [One of the main authors of MDX recommends `remark-cli` with `remark-mdx`](https://github.com/orgs/mdx-js/discussions/2067).
...
If you [swizzle](../../swizzling.md) this component, you will find all tags that have been implemented, and you can further customize our implementation by swizzling the respective sub-component, like `@theme/MDXComponents/Code` (which is used to render [Markdown code blocks](./markdown-features-code-blocks.md)).
...
import React from 'react';
import FeatureDisplay from './_featureDisplay.md';
// highlight-next-line
...
The [MDX syntax](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax) is mostly compatible with [CommonMark](https://commonmark.org/), but is much stricter because your `.md` files can use JSX and are compiled into real React components (check the [playground](https://mdxjs.com/playground/)).
...
See [using code blocks in JSX](./markdown-features-code-blocks.md#usage-in-jsx) for more details of the `<CodeBlock>` component.
...
```md title="_markdown-partial-example.md"
...
{/* prettier-ignore */}
```jsx title="someOtherDoc.md"
import PartialExample from './_markdown-partial-example.md';
...
- `toc`: the table of contents, as a tree of headings. See also [Inline TOC](./markdown-features-toc.md#inline-table-of-contents) for a more concrete use-case.
