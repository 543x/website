...
We only copy `.md` and `.md` files, as React pages are translated through JSON translation files already.
...
For localized sites, it is recommended to use **[explicit heading IDs](../guides/markdown-features/markdown-features-toc.md#heading-ids)**.
...
You can now [deploy](../deployment.md) the `build` folder to the static hosting solution of your choice.
...
On your [static hosting provider](../deployment.md):
...
We will share two common translation collaboration strategies: [**using git**](./i18n-git.md) and [**using Crowdin**](./i18n-crowdin.md).
