...
The `<Translate>` component supports interpolation. You can also implement [string pluralization](https://github.com/facebook/docusaurus/pull/i18n/i18n-tutorial.md#pluralization) through some custom code and the [`translate` imperative API](#translate-imperative).
...
Prefer a `require()` call for [assets](./guides/markdown-features/markdown-features-assets.md):
