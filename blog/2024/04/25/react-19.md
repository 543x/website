...
In React 19 we're ducing a new API to read resources in render: `use`.
...
Due to the duction of ref cleanup functions, returning anything else from a `ref` callback will now be rejected by TypeScript. The fix is usually to stop using implicit returns, for example:
